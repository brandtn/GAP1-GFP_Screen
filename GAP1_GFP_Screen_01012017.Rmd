---
title: "Gresham Lab Flow Core Guide - Nathan's Version"
author: '`r Sys.info()[7]`'
date: '`r Sys.Date()`'
output:
  html_document:
...
This is to establish and explore a standardized flow cytometry analysis using Nathan's favorized coding methods and styles.
###############################
...
In order to run this code you need:  
  ...
  *NEED TO WORK ON GATING ASAP
  
  + the path of the directory(ies), given the variable names **dir1**, **dir2**... that contain .fcs files named A01.fcs, A02.fcs, A03.fcs...  
  *This should be able to utilize a loop, especially if a single directory format is utilized
  
  + a tab delimited sample sheet in each directory with the following rows: <Well>	<Strain>	<Genotype>	<Ploidy>	<Media>	<Experiment>  
  *This contradics the code below, but it is a pretty good setup for the sample sheet. Look into if its better to do one sample sheet or multiple. Well I need a varible defining the number of samples in the sample sheet???
...
#Variables for scanning GAP1::GFP Screen
  plate <- "01" #Plate number, under 10 need 0#
...
dir1 <- paste(path.data,"GAP1_GFP_Screen_FSC_Files/Plate",plate,"_",condition, sep="")
dir2 <- paste(path.data,"GAP1_GFP_Screen_FSC_Files/Plate",plate,"_",condition, sep="")
...
sample.sheet.1 <- read.csv(paste(path.data,"GAP1_GFP_Screen_DataSheets/Plate",plate,"_",condition,".csv", sep=""))
sample.sheet.2 <- read.csv(paste(path.data,"GAP1_GFP_Screen_DataSheets/Plate",plate,"_",condition,".csv", sep=""))

#Change names of samples to those specified in the sample sheets
sampleNames(flowData.1) <- paste(sample.sheet.1[,1], sample.sheet.1[,2], sample.sheet.1[,3], sample.sheet.1[,4], sample.sheet.1[,5], sep=" ") #sample.sheet.1[,6], sep=" ")
sampleNames(flowData.2) <- paste(sample.sheet.2[,1], sample.sheet.2[,2], sample.sheet.2[,3], sample.sheet.2[,4], sample.sheet.2[,5], sep=" ") #sample.sheet.2[,6], sep=" ")
